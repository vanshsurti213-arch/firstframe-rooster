...
async function run() {
  ...
  const lines = rawNicheData.trim().split('\n').filter(l => l.trim().length > 0);
  ...
  for (const line of lines) {
    ...
    const rawNiches = parts[8] || parts[7]; // sometimes instagram is missing, shifting columns
    
    // Attempt to extract the correct niches column. Usually it contains "Beauty", "Lifestyle" etc.
    let nicheString = '';
    for (const p of parts) {
       if (p.includes('Beauty') || p.includes('Lifestyle') || p.includes('Fashion')) {
           nicheString = p;
           break;
       }
    }
    
    if (!nicheString) continue;

    const parsedNiches = nicheString.split(',').map(n => n.trim()).filter(n => n && !n.toLowerCase().includes('ugc'));
    ...
    if (match) {
        ...
        const currentNichesStr = (match.niches || []).join(',');
        const newNichesStr = parsedNiches.join(',');
        if (currentNichesStr !== newNichesStr) {
           match.niches = parsedNiches;
           updatedCount++;
           console.log(`✅ Updated Niches for ${match.name}:`, parsedNiches);
        }
    }
  }

  // Also ensure UGC is stripped from EVERYONE else just in case
  for (const c of creators) {
     if (c.niches) {
        const orig = c.niches.length;
        c.niches = c.niches.filter(n => !n.toLowerCase().includes('ugc'));
        if (c.niches.length !== orig && !lines.some(l => l.toLowerCase().includes(c.name.toLowerCase()))) {
           updatedCount++;
           console.log(`✅ Stripped UGC from ${c.name}`);
        }
     }
  }
  ...
  if (updatedCount > 0) {
    ...
    console.log('✅ Niche sync complete!');
  } else {
